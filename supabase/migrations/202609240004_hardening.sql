-- Operational hardening: retries, source refresh continuity, and job ownership controls.

create policy versions_update_doc on public.document_versions for update to authenticated
using (
  exists(
    select 1 from public.documents d
    where d.id=document_id and d.organization_id is not null
      and (d.owner_user_id=(select auth.uid()) or (select private.is_org_admin(d.organization_id)))
  )
)
with check (
  exists(
...
create policy jobs_update_owner_admin on public.ingestion_jobs for update to authenticated
using (
  exists(
    select 1
    from public.document_versions v
    join public.documents d on d.id=v.document_id
    where v.id=document_version_id and d.organization_id=organization_id
      and (d.owner_user_id=(select auth.uid()) or (select private.is_org_admin(d.organization_id)))
  )
)
with check (
...
-- Keep the previous ready version searchable while a replacement version is being fetched/parsed.
