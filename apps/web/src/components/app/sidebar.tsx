...
function NavigationContent({
  ...
  async function setArchived(conversation: ConversationNavItem, archived: boolean) {
    ...
    try {
      ...
      if (archived && currentConversation === conversation.id) {
        router.push(`/app/bots/${conversation.bot_id}`);
      }
      router.refresh();
  ...
  async function deleteConversation(conversation: ConversationNavItem) {
    ...
    const busyKey = `delete:${conversation.id}`;
    setConversationBusy(busyKey);
    try {
  ...
  function ConversationRow({ conversation, archived = false }: { conversation: ConversationNavItem; archived?: boolean }) {
    const working = conversationBusy?.endsWith(`:${conversation.id}`) ?? false;
    return (
      <div className="group flex items-center rounded-lg transition hover:bg-white/[.045]">
        <Link
          href={`/app/bots/${conversation.bot_id}?conversation=${conversation.id}`}
          onClick={onNavigate}
          className="flex min-w-0 flex-1 items-start gap-2.5 px-2.5 py-2"
        >
          <MessageSquare className="mt-0.5 size-3.5 shrink-0 text-[#7f8a9d]" />
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[12px] text-[#b8c2d0]">{conversation.title || "Conversation"}</span>
        ...
        <div className="mr-1 flex shrink-0 items-center gap-0.5 opacity-70 transition sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">
          <button
            ...
            onClick={() => void setArchived(conversation, !archived)}
            className="grid size-7 place-items-center rounded-md text-[#7f8a9d] transition hover:bg-white/[.07] hover:text-foreground disabled:opacity-40"
          >
          ...
          <button
            ...
            className="grid size-7 place-items-center rounded-md text-[#7f8a9d] transition hover:bg-red-500/[.09] hover:text-red-300 disabled:opacity-40"
          >
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#090c12]">
      <div className={cn("flex h-[66px] shrink-0 items-center justify-between", collapsed ? "px-2" : "px-3.5")}>
        <Link href="/app" onClick={onNavigate} aria-label="Provenance home" title={collapsed ? "Provenance" : undefined}>
          {collapsed ? <BrandMark priority className="size-10" /> : <BrandLockup priority markClassName="size-10" textClassName="text-[20px]" />}
        </Link>
        {onClose ? (
          ...
          <button
            ...
            className="grid size-8 place-items-center rounded-lg text-[#7f8a9d] transition hover:bg-white/[.06] hover:text-foreground"
          >
      ...
      <div className={cn("shrink-0 pb-2", collapsed ? "px-2" : "px-2.5")}>
        <Link
          ...
          className={cn(
            "flex h-10 items-center rounded-lg border border-[#2d3d56] bg-[#111722] text-[13px] font-medium text-[#e8eef8] transition hover:border-[#42679f] hover:bg-[#141c29]",
            collapsed ? "justify-center px-0" : "gap-2.5 px-3",
          )}
        >
          <Plus className="size-4 shrink-0 text-[#8bb7ff]" />
          {!collapsed ? "New custom assistant" : null}
        </Link>
      </div>
      ...
      <div className={cn("min-h-0 flex-1 overflow-y-auto overscroll-contain pb-3", collapsed ? "px-2" : "px-2.5")}>
        ...
        {collapsed ? (
          <div className="mt-4 space-y-1 border-t border-white/[.06] pt-3">
            {ASSISTANT_CATALOG.map((preset) => {
              ...
              return (
                <NavLink key={preset.key} href={href} active={active} collapsed onNavigate={onNavigate} title={preset.name}>
        ...
        {!collapsed ? (
          <div className="mt-5">
            <div className="px-2.5 pb-1.5 text-[11px] font-medium text-[#7f8a9d]">Recent chats</div>
            <div className="space-y-0.5">
              {activeConversations.length ? activeConversations.slice(0, 24).map((conversation) => (
                <ConversationRow key={conversation.id} conversation={conversation} />
              )) : (
                <p className="px-2.5 py-2 text-xs leading-5 text-[#737f91]">Chats appear here after you send the first message.</p>
              )}
            </div>

            {archivedConversations.length ? (
              <details className="group mt-3">
                <summary className="flex cursor-pointer list-none items-center justify-between rounded-lg px-2.5 py-2 text-[11px] font-medium text-[#707c8e] transition hover:bg-white/[.035] hover:text-[#aeb9c8]">
                  <span>Archived · {archivedConversations.length}</span>
                  <ChevronDown className="size-3.5 transition group-open:rotate-180" />
                </summary>
                <div className="mt-1 space-y-0.5">
                  {archivedConversations.slice(0, 24).map((conversation) => (
                    <ConversationRow key={conversation.id} conversation={conversation} archived />
                  ))}
                </div>
              </details>
            ) : null}
      ...
      <div className={cn("shrink-0 border-t border-white/[.07]", collapsed ? "p-2" : "p-2.5")}>
        {collapsed ? (
          <div className="flex flex-col items-center gap-1.5 py-1">
            <div title={email || "Signed in"} className="grid size-8 place-items-center rounded-full bg-[#162238] text-xs font-semibold text-[#d8e6ff]">
              {(email?.[0] || "P").toUpperCase()}
            </div>
            <Link href="/app/settings" onClick={onNavigate} aria-label="Settings" title="Settings" className="grid size-8 place-items-center rounded-lg text-[#8e9aab] hover:bg-white/[.055] hover:text-foreground">
              <Settings className="size-4" />
            </Link>
            <button onClick={signOut} disabled={signingOut} aria-label="Sign out" title="Sign out" className="grid size-8 place-items-center rounded-lg text-[#8e9aab] hover:bg-white/[.055] hover:text-foreground disabled:opacity-50">
          ...
          <div className="flex items-center gap-2 rounded-xl px-2 py-2">
            ...
            <Link href="/app/settings" onClick={onNavigate} aria-label="Settings" title="Settings" className="grid size-8 shrink-0 place-items-center rounded-lg text-[#8e9aab] hover:bg-white/[.055] hover:text-foreground">
              <Settings className="size-4" />
            </Link>
            <button onClick={signOut} disabled={signingOut} aria-label="Sign out" title="Sign out" className="grid size-8 shrink-0 place-items-center rounded-lg text-[#8e9aab] hover:bg-white/[.055] hover:text-foreground disabled:opacity-50">
              <LogOut className="size-4" />
            </button>
          </div>
        )}
...
export function Sidebar({ assistants, conversations, email }: { assistants: AssistantNavItem[]; conversations: ConversationNavItem[]; email?: string }) {
  ...
  return (
    <>
      <aside className={cn("sticky top-0 hidden h-dvh shrink-0 border-r border-white/[.07] transition-[width] duration-200 lg:block", collapsed ? "w-[72px]" : "w-[260px]")}>
        <NavigationContent
